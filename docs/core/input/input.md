...
| std::vector&lt;Touch&gt; | [**getTouches**](input-get-touches.md) | | All active touch contacts, each with a `pointer` id and a `position`. |
...
| std::vector&lt;Gamepad&gt; | [**getGamepads**](input-get-gamepads.md) | | All connected controllers with their button and axis state. |
